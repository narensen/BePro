    ...
    <div className="fixed inset-0 backdrop-blur-lg pl-72 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 rounded-2xl p-6 max-w-md w-full mx-4 max-h-[80vh] overflow-hidden">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-black text-gray-300">{title}</h2>
          <button
            onClick={onClose}
            className="text-gray-300 hover:text-gray-400 text-xl"
          >
            ×
          </button>
        </div>
        ...
        <div className="overflow-y-auto max-h-[60vh] space-y-4">
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-200">No {title.toLowerCase()} yet</p>
            </div>
          ) : (
            data.map((item) => {
              ...
              return (
                <div key={item.id} className="flex items-center space-x-4 p-3 hover:bg-gray-800 rounded-lg transition-colors">
                  <div className="w-12 h-12 bg-gradient-to-br from-gray-200 to-gray-600 rounded-full flex items-center justify-center text-gray-900 font-bold">
                    {profile.avatar_url ? (
                  ...
                  <div className="flex-1">
                   <Link href={`https://bepro.live/${profile.username}`}>
                    <h3 className="text-gray-300 font-bold">{profile.username}</h3>
                    </Link>
