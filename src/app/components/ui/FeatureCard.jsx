...
export const FeatureCard = ({ icon, title, description, delay = 0 }) => {
  ...
  return (
    <div ref={ref} className={`relative p-8 rounded-3xl backdrop-blur-xl border border-white/20 bg-gradient-to-br from-white/10 to-white/20 transition-all duration-700 hover:scale-105 hover:border-amber-400/50 group cursor-pointer ${ isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${delay}ms` }} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
