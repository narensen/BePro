...
// import { supabase } from './lib/supabase_client' // ◀️ 1. IMPORT YOUR SUPABASE CLIENT
// import useUserStore from './store/useUserStore' // ◀️ 2. IMPORT YOUR USER STORE (e.g., Zustand)

// MOCK IMPORTS (replace with your actual files)
const supabase = { auth: { getSession: async () => ({ data: { session: null } }), signOut: async () => console.log("Signed out") } };
const useUserStore = () => {
    const [store, setStore] = useState({ user: null, setUserSession: (u) => setStore(s => ({...s, user: u})), clearUserSession: () => setStore(s => ({...s, user: null})) });
    return store;
};
// END MOCK IMPORTS
...
const InteractiveDemo = () => {
  ...
  useEffect(() => {
    ...
    setCurrentFeedback(matchedRule);
  }, [code]);
...
// --- Main App Component with Integrated Functionality ---
export default function LandingPage() {
  const [loading, setLoading] = useState(true);
  const { user, setUserSession, clearUserSession } = useUserStore(); // ◀️ FROM YOUR OLD CODE
  const router = useRouter(); // ◀️ FROM YOUR OLD CODE
  const mousePosition = useMousePosition();

  // ◀️ 3. AUTHENTICATION CHECK LOGIC
  useEffect(() => {
      ...
      <header className="fixed top-0 left-0 right-0 z-50 p-6">
        <div className="backdrop-blur-2xl bg-white/10 border border-white/20 rounded-2xl px-8 py-4 mx-auto max-w-7xl shadow-2xl">
          <div className="flex justify-between items-center">
            ...
            <div>
              {/* ◀️ 6. DYNAMIC HEADER BUTTONS */}
              {user ? (
                <div className="flex items-center gap-4">
                    <button onClick={() => router.push('/home')} className="px-6 py-3 font-bold bg-gray-900 text-amber-300 rounded-xl hover:bg-gray-800 transition-all duration-300 hover:scale-105 shadow-lg">
                      Dashboard
                    </button>
                    <button onClick={handleSignOut} className="px-4 py-3 font-bold text-gray-800 hover:text-gray-900 transition-colors">
                      Sign Out
                    </button>
                </div>
              ) : (
      ...
      <section className="min-h-screen flex items-center justify-center relative pt-20">
        ...
        <div className="relative z-10 text-center px-4 max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center items-center animate-fade-in-up" style={{ animationDelay: '0.8s' }}>
            <button
              onClick={handleAuthAction} // ◀️ DYNAMIC ACTION
      ...
      <AnimatedSection id="codex" className="py-32 px-4" background="bg-white/10 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <FeatureCard icon={<CodexIcon />} title="Deconstruction" description="The Codex analyzes thousands of data points to generate a master blueprint of the exact skills and projects required for your target role." delay={0} />
            <FeatureCard icon={<CodexIcon />} title="Prescription" description="You receive a single, focused mission each week. The Codex provides the curated resources you need to win. Your only job is to execute." delay={200} />
            <FeatureCard icon={<CodexIcon />} title="Validation" description="As you complete missions, your AI Mentor evaluates your work, pushing you to improve. Your BePro profile becomes a living, validated record of your mastery." delay={400} />
          </div>
        </div>
      </AnimatedSection>
      ...
      <AnimatedSection id="fortress" className="py-32 px-4" background="bg-gray-900/20 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-8">
                ...
                <FeatureCard icon={<FortressIcon />} title="Inter-Community Raids" description="Participate in challenges and collaborative roadmaps against other universities to prove who forges the best commanders." delay={200} />
                <FeatureCard icon={<FortressIcon />} title="Recruit Your Allies" description="Find your future co-founders and collaborators within a high-trust network of the most ambitious builders on your campus." delay={400} />
            </div>
        </div>
      </AnimatedSection>
