...
const FortressIcon = ({ className = "" }) => (
  <div className={`relative ${className}`}>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" className="relative h-12 w-12 text-orange-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const FeatureCard = ({ icon, title, description, delay = 0 }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-2 right-2 w-2 h-2 bg-amber-400 rounded-full transition-all duration-300 ${isHovered ? 'scale-150 opacity-100' : 'scale-100 opacity-50'}`} />
      <div className={`absolute bottom-2 left-2 w-2 h-2 bg-orange-400 rounded-full transition-all duration-300 ${isHovered ? 'scale-150 opacity-100' : 'scale-100 opacity-50'}`} />
    </div>
  );
};
