export default function SortOptions({ sortMode, setSortMode }) {
  ...
  return (
    <div className="flex gap-1 mb-4 border-b border-gray-200/50">
      {sortOptions.map((mode) => (
        <button
          key={mode.key}
          onClick={() => setSortMode(mode.key)}
          className={`px-4 py-3 text-sm font-bold transition-all duration-200 border-b-2 hover:bg-gray-100/50 ${
            sortMode === mode.key
              ? 'border-amber-500 text-amber-700 bg-gray-100/50'
